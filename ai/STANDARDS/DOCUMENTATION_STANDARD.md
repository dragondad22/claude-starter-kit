...
- **Central error reference:** if the product uses error codes or catches
  ...
  error reference inside `README.md, plus the shipped guide in template/core/docs/kit/`: the code, what happened,
...
> **Source of truth for this project:** `README.md, plus the shipped guide in template/core/docs/kit/`
...
1. **Reference manual / long form** — beginner-level, example-rich, one page per
   screen or command plus task tutorials. Lives in `README.md, plus the shipped guide in template/core/docs/kit/`.
...
is not steeped in AI-assisted software development process jargon hesitate at this field?** If yes, add
...
- Generate them with your E2E tool against a stable demo/seed dataset
  (`N/A (no runtime UI)`) rather than capturing by hand — hand-captured shots rot.
