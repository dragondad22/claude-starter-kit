...
- Full / primary test suite: `bash scripts/selftest.sh`
- End-to-end / browser suite (if any): `N/A (no runtime UI)`
- Build / typecheck (must pass before tests are meaningful): `N/A (no build step)`
- Data-layer codegen / migration prep (if the `N/A` requires it before tests run):
...
- **Backend changes:** run any required data-layer codegen, then `bash scripts/selftest.sh` for the backend.
- **Frontend changes:** run the frontend unit suite (and `N/A (no runtime UI)` if the change is user-visible flow).
...
`Markdown (the product) + POSIX shell and Python 3 (tooling)` and test file conventions.
...
1. Retry after environment repair (**max 2 attempts**), in this order:
   ...
   - Re-run any required data-layer codegen / migration step for `N/A`.
...
- `testing-reports/artifacts/<date>_<KIT-NNN>_<feature>_<run>/`
...
> `bash scripts/selftest.sh` / `N/A (no runtime UI)` for this project, or run the commands directly.
