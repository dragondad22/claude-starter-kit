...
3. **Generate the founding artifacts from `Final:` fields** — every derived artifact gets a `Source:` citation (qualified Q-ID, e.g. `000/Q-ARCH-03`) and is listed back on the question's `Derived:` line and in `00-INDEX.md`:
   ...
   - **Product register**: seed `docs/registers/PRODUCT_REGISTER.md` with the application-scope rules the answers already settle — business rules the domain imposes (`BR-`), capabilities the product must provide (`PR-`), any stated quality targets (`NFR-`), isolation/access rules that must hold under every request (`INV-`), and product-wide wording rules (`UX-`). Each row gets today's date and a `Source:` Q-ID. Route anything failing the register's routing rule to the home that rule names — an architecture choice becomes an ADR, not a register row. Remove the worked-example comment blocks once real rows exist.
