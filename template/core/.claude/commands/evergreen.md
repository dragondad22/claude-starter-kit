...
2. **Run the seven lenses** against the repo and the history since the last review (`git log`, closed issues/PRs):
   ...
   - **Standards drift** — do the written standards still match actual practice? Fix the standard or fix the practice — deliberately, either way. Includes orphaned setup blocks: every "Setup (once per repo)" section must be reachable from `/bootstrap` or `/conform` (rule: `ai/STANDARDS/DOCUMENTATION_STANDARD.md` § Setup blocks). Also leftover genericization banners: a standard/template with no unfilled `{{TOKENS}}` still carrying the kit's line-1 banner → strip it (banner semantics: `bootstrap/PLACEHOLDERS.md` § Meta-literals).
   - **Date sweep** — currency-sensitive rows: paved-road "Last reviewed" dates, compliance register "Verified" dates, and every standard's mandatory `Last Updated` line where it no longer reflects the file's content (rule + why it isn't redundant with version control: `ai/STANDARDS/DOCUMENTATION_STANDARD.md` § Every standard states when it was last updated). A reader holding a distributed copy has only the stated date to go on.
   ...
   - **Cleared blockers** — deferrals whose precondition has since been met. A deferral names something it is waiting on, and then nothing re-reads it: the blocker lands and the record goes on saying "later" indefinitely. Sweep the places a project parks work — interview questions still `deferred` / `needs-research` / `waiting-on-<who>` (`ai/STANDARDS/INTERVIEW_STANDARD.md` § Question Lifecycle), ADR and register entries that say "revisit when X", **Open questions** rows in `docs/specs/`, and issues blocked by an issue that has since closed — then check whether the named precondition actually landed. Report each with **the date it was met**, so the gap between *unblocked* and *noticed* is visible instead of silent. A `deferred` question deserves a second look on its own: its default was accepted under conditions that may no longer hold.
...
4. **File the outputs — findings never live only in conversation:**
   ...
   - Adoption/change decisions (when the issue is worked) → an ADR or a register row, as any decision.
