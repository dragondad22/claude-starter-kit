...
| UI-driving for native mobile | **A pair, not one tool**: a **playback** runner + an **interactive** driver | Independent review and regression of a native mobile UI need both halves of the driver seam, and no single mobile tool gives both. **Playback** (committed regression): Flutter → **Patrol** (native, builds on `flutter_test`) or **Maestro** (black-box, no code changes). **Interactive** (agent-driven discovery/review): an accessibility-tree/emulator driver. **Prerequisite:** the app must expose a real semantics tree (labels + stable keys) — the *same work* as screen-reader accessibility (`ai/STANDARDS/UI_STANDARD.md`); without it neither driver can reliably find elements. Note: Maestro is a *playback* option only — it cannot be steered live for discovery. Deviations → ADR. | 2026-08-03 |
