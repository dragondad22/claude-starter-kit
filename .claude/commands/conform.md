...
2. **Inventory drift against the standards that define "conformant".**
   ...
   - Doc layout → the shipped tree: ADRs in `docs/architecture/decisions/`, specs in `docs/specs/`, runbooks in `docs/runbooks/`, interviews/plans in `docs/plans/`, registers in `docs/registers/`, glossary/personas at `docs/` root.
   - Structure → standards in `ai/STANDARDS/`, templates in `ai/TEMPLATES/`, scripts in `ai/scripts/`, checklists in `ai/CHECKLISTS/`.
   - **An existing decision log** — whatever this repo calls it: a `decision-log.md` at the docs root, a `decisions/`, `adr/`-adjacent or `rfc/` directory, a wiki page → the kit no longer ships one: a decision is an *event*, and what it produces belongs in an ADR or a register row. Route its entries through the register's routing rule and propose the destination for each — architecture to an ADR (**check first: many logs restate an ADR that already exists**, so the entry disappears rather than moving), rules and requirements to `docs/registers/`, terms to the glossary, scope calls to the tracker. Reasoning that isn't captured anywhere else moves to a `docs/plans/` working doc the new record cites. **Never delete the log as part of this** — leave it in place, stamped historical, until the human confirms every entry has landed. A routing that can't be decided is a finding for the human, not a guess.
